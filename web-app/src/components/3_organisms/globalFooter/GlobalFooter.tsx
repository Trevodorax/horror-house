      ...
      <Surface className={styles.footerSurface} background="secondary">
        <Surface background="transparent" className={styles.footerNav}>
          {navItems.map((item, index) => (
            <LinkListItem key={`${item.label}-${index}`} customClassNames={{link: styles.link}} item={item} />
          ))}
        </Surface>
        ...
        <Surface className={styles.socialsContainer} background="transparent">
          <Link to='https://www.instagram.com'><InstagramIcon className={styles.instagramIcon} /></Link>
          <Link to='https://www.twitter.com'><TwitterIcon className={styles.twitterIcon} /></Link>
        </Surface>
      </Surface>
    </footer>
  );
