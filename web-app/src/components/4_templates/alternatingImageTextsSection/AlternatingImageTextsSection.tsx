			...
			<Surface background="transparent" className={styles.sectionsContainer}>
				{imageTexts.map(({ imageUrl, text }, index) => (
					<Surface
						...
						key={`${imageUrl}-${text.slice(0, 3)}`}
						className={classNames(styles.textImageSection, {
							[styles.reversed]: index % 2 === 1,
						})}
					>
						<img className={styles.image} src={imageUrl} alt="sectionImage" />
